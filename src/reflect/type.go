			...
			for i := uintptr(0); i < ktyp.size/ptrSize; i++ {
				if (kmask[i/8]>>(i%8))&1 != 0 {
					for j := uintptr(0); j < bucketSize; j++ {
						word := (bucketSize+j*ktyp.size)/ptrSize + i
		...
		if etyp.kind&kindNoPointers == 0 {
			...
			for i := uintptr(0); i < etyp.size/ptrSize; i++ {
				if (emask[i/8]>>(i%8))&1 != 0 {
					for j := uintptr(0); j < bucketSize; j++ {
						word := (bucketSize+bucketSize*ktyp.size+j*etyp.size)/ptrSize + i
						mask[word/8] |= 1 << (word % 8)
					}
				}
			}
		}
		word := bucketSize * (1 + ktyp.size + etyp.size) / ptrSize
		mask[word/8] |= 1 << (word % 8)
		gcdata = &mask[0]
...
func ArrayOf(count int, elem Type) Type {
	...
	*array = *prototype
	...
	case typ.kind&kindGCProg == 0 && array.size <= 16*8*ptrSize:
		...
		mask := make([]byte, (array.size/ptrSize+7)/8)
		elemMask := (*[1 << 30]byte)(unsafe.Pointer(typ.gcdata))[:]
		elemPtrs := typ.size / ptrSize
		for j := uintptr(0); j < elemPtrs; j++ {
		...
		array.gcdata = &mask[0]
		array.ptrdata = array.size // overestimate but okay
	...
	default:
		...
		if typ.kind&kindGCProg == 0 {
			...
			elemProg := elemGC[:*(*uint32)(unsafe.Pointer(&elemGC[0]))]
			prog = append(prog, elemProg...)
		}
		elemWords := typ.size / ptrSize
		if elemPtrs < elemWords {
			prog = append(prog, 0x01, 0x00, 0x81)
			prog = appendVarint(prog, elemWords-elemPtrs)
		}
		if elemWords < 0x80 {
		...
		prog = appendVarint(prog, uintptr(count))
		prog = append(prog, 0)
