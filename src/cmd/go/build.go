...
func (b *builder) build(a *action) (err error) {
	...
	// Compile Go.
	ofile, out, err := buildToolchain.gc(b, a.p, a.objpkg, obj, inc, gofiles, false)
	if len(out) > 0 {
		b.showOutput(a.p.Dir, a.p.ImportPath, b.processOutput(out))
		if err != nil {
			return errPrintedOutput
...
func (b *builder) installForShared(a *action) (err error) {
	...
	err = ioutil.WriteFile(dsomarker, []byte(a1.p.SharedLib + "\n"), 0644)
	if err != nil {
		return err
	}
...
type toolchain interface {
	...
	gc(b *builder, p *Package, archive, obj string, importArgs []string, gofiles []string, forShared bool) (ofile string, out []byte, err error)
...
func (noToolchain) gc(b *builder, p *Package, archive, obj string, importArgs []string, gofiles []string, forShared bool) (ofile string, out []byte, err error) {
...
func (gcToolchain) gc(b *builder, p *Package, archive, obj string, importArgs []string, gofiles []string, forShared bool) (ofile string, output []byte, err error) {
	if archive != "" {
		ofile = archive
	} else {
		out := "_go_." + archChar
...
func (b *builder) swigIntSize(obj string) (intsize string, err error) {
	...
	if _, _, e := buildToolchain.gc(b, p, "", obj, nil, srcs, false); e != nil {
		return "32", nil
	}
	return "64", nil
}
