...
func (tools gccgoToolchain) ld(b *builder, p *Package, out string, allactions []*action, mainpkg string, ofiles []string) error {
	...
	//   1) prefer a fake package (created by (*builder).test) to a real package
	...
	for i := len(allactions) - 1; i >= 0; i-- {
		a := allactions[i]
		if a.p != nil && a.p.fake && !importPathsSeen[a.p.ImportPath] {
	...
	for i := len(allactions) - 1; i >= 0; i-- {
		a := allactions[i]
		if a.p != nil && !a.p.Standard && !importPathsSeen[a.p.ImportPath] {
			importPathsSeen[a.p.ImportPath] = true
			afiles = append(afiles, a.target)
		}
	}
