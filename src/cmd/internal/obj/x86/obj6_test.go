...
MOVQ name(SB), AX -> MOVQ name@GOT(SB), R13; MOVQ (R13), AX
MOVQ name+10(SB), AX -> MOVQ name@GOT(SB), R13; MOVQ 10(R13), AX

CMPQ name(SB), $0 -> MOVQ name@GOT(SB), R13; CMPQ (R13), $0

MOVQ $1, name(SB) -> MOVQ name@GOT(SB), R13; MOVQ $1, (R13)
MOVQ $1, name+10(SB) -> MOVQ name@GOT(SB), R13; MOVQ $1, 10(R13)
`
...
func asmOutput(t *testing.T, s string) []byte {
	...
	cmd := exec.Command(
		build.Default.GOROOT+"/bin/go", "tool", "asm", "-S", "-buildmode=shared",
		"-o", filepath.Join(tmpdir, "output.6"), tmpfile.Name())
	cmd.Env = os.Environ()
	cmd.Env = append([]string{"GOARCH=amd64"}, cmd.Env...)
