...
// dgcsym emits and returns a symbol containing GC information for type t,
...
func dgcptrmask(t *Type) *Sym {
	...
	if sym.Flags&SymUniq == 0 {
		...
		ggloblsym(sym, maxPtrmaskBytes, obj.DUPOK|obj.RODATA|obj.LOCAL)
	}
	return sym
}
...
// In practice, the size is typeptrdata(t) except for non-trivial arrays, which end up mapping t.Width.
func dgcprog(t *Type) (*Sym, int64) {
	...
	p.init(sym)
	p.emit(t, 0)
	offset := p.w.Offset() * int64(Widthptr)
	p.end()
...
func (p *GCProg) emit(t *Type, offset int64) {
	...
	case TARRAY:
		...
		if count == 1 || count*elem.Width/int64(Widthptr) < 4*8 {
		...
		p.emit(elem, offset)
		p.w.Advance((offset + elem.Width) / int64(Widthptr))
